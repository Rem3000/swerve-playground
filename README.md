...
python test_limits.py    # 限位安全測試，另外還有 test_model/homing/estop/input/parity/params
...
| `test_*.py` | 七支獨立測試，見下表 |
| `ref_kinematics.py` | 從 `SwerveKinematics.cpp` 獨立重譯的參考實作，只給 `test_parity.py` 用 |
...
| 檔案 | 驗什麼 |
|---|---|
| `test_model.py` | 運動學與里程計 |
| `test_limits.py` | 限位安全：會不會撞壞微動開關 |
| `test_homing.py` | 開機歸零精度 |
...
>
> 撞塊偏移（`STRIKER_OFFSET_DEG = +90°`）是撞塊銷相對輪子朝向的角度差，由 CAD
> 對圖確認，四個模組同值。它**不進入任何運算** —— 機械角度的刻度
> （0 = 碰 min 開關、270 = 碰 max 開關）本來就吸收掉了機構細節 —— 純粹是為了把
> 限位機構畫在正確的位置。但它和「量到的開關位置」一起決定了 `MOUNT_OFFSET`
> （那個**會**進運算），所以填錯不是只有圖畫歪而已，見 README 第 7 節。
>
> 算出來的死角剛好朝車體中心，正是開關裝在內側的必然結果，可以拿來當佐證。
...
| 指令 | 四顆機械角 | 說明 |
|---|---|---|
| 原地自轉（順逆都一樣）| 全部 ≈ 135° | 只差輪速正負；135° 正好是 park，離兩顆開關都最遠 |
| 直線前進 | FL 180°、FR 90°、RL 90°、RR 180° | FR / RR 會用反向解 |

四顆相同是「四個模組同一種裝法、繞車體中心 90° 陣列」的必然結果。自轉剛好落在
行程正中間是這組安裝角度帶來的好處 —— 自轉是最常用的動作，而它離兩顆開關最遠。

某一顆拿到 θ（正解）或 θ±180（反向解）都對 —— 兩者差 180° 又把輪速取負，物理上是
同一件事，程式挑離目前角度近、而且避開 keep-out 的那個。
