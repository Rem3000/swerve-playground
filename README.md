...
操控：右邊搖桿平移（放開回中，右手拇指剛好落在上面）、左邊橫條自轉、
`×0.6` 切速度倍率、`重新歸零` 真的跑一次歸零狀態機、`急停` 是閂鎖式
（放開後還要零速一秒才解除，和韌體一樣）。

左上角的按鈕切換兩個視圖：

- **底盤**：每個模組一個圓，是撞塊的軌跡。灰色是能走的 270°、暗紅是到不了的
  90° 死角、紅短線是兩顆微動開關、圓點是撞塊現在的位置（越接近開關越紅）、
  藍虛線是韌體下的目標角、粗線是輪子朝向（綠 = 正轉、紅 = 反轉）。
- **路徑**：綠色是車子真正走的軌跡。按 `里程計` 才疊上橘色那條「里程計以為
  自己走的」，兩條分岔多遠就是飄掉多少（橘色虛線車體 = 里程計推算的位置）。
  視野會自動縮放把要顯示的軌跡都框進畫面，`清除` 重新開始記。
  軌跡點是在瀏覽器這邊累積的，不是從模型每幀傳上千個點過來。

桌面版 `swerve_sim.py` 的掃描自檢（`sweep_cmd`）還沒搬過去。
