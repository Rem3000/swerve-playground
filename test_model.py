...
# 安裝偏移是 -180/+90/-90/0，扣掉之後純前進的機械角只會是 90 或 180
# （FL=180 FR=90 RL=90 RR=180，其中 FR/RR 是用反向輪速的解）
ok_mech = all(any(abs(st[i][0] - v) < 1e-6 for v in (90.0, 180.0)) for i in range(N))
check(ok_mech, f"機械角落在等價解集合上，實得 {[round(s[0], 1) for s in st]}")

k2 = new_kin()
st = k2.inverse(0.0, 0.0, 2.0, [180.0] * N)  # 原地逆時針
got = [round(s[0], 3) for s in st]
# 四顆相同是「四個模組同一種裝法、繞車體中心 90 度陣列」的必然結果；
# 剛好落在 135 度（park、離兩顆開關都最遠）則是這組安裝角度帶來的好處
check(got == [135.0] * N,
      f"原地自轉時四顆機械角都是 135 度（park），實得 {got}")
